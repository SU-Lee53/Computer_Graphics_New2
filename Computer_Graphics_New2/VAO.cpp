...
VAO::VAO(float* vertex, float* normal, float* color, unsigned int* element, int vBufSize, int eBufSize)
{
	this->_vbo.PushToVertexBuffer(vertex, _vBufSize);
...
void VAO::InitVAO(VAO_TYPE type)	// elementMode가 true면 EBO를 사용하므로 버퍼 3개 생성함
{
	...
	if(type != NORMAL_AND_TEXTURE)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, _vBufSize * sizeof(float), &_vbo.GetVertexBuf().front(), GL_DYNAMIC_DRAW);
		if (type == INDEXED)	// element 버퍼 사용시에는 vbo 하나 더 사용
		{
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, *_vbo.GetElementHandle());
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, _eBufSize * sizeof(unsigned int), &_vbo.GetElementBuf().front(), GL_DYNAMIC_DRAW);
		}
		...
		glBufferData(GL_ARRAY_BUFFER, _vBufSize * sizeof(float), &_vbo.GetColorBuf().front(), GL_DYNAMIC_DRAW);
	...
	else
	{
		...
		glBufferData(GL_ARRAY_BUFFER, _vBufSize * sizeof(float), &_vbo.GetNormalBuf().front(), GL_DYNAMIC_DRAW);
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
